...
export class DiscordResonance extends Resonance {
  ...
  public async getLocale(): Promise<string> {
    ...
    const i18nUserConfig = await Core.gestalt().get(`/i18n/${this.bot.id}/clients/discord/users`)
      .catch(Igor.stop);
    ...
    const i18nChannelConfig = await Core.gestalt().get(`/i18n/${this.bot.id}/clients/discord/channels`)
      .catch(Igor.stop);
    ...
    const i18nGuildConfig = await Core.gestalt().get(`/i18n/${this.bot.id}/clients/discord/guilds`)
      .catch(Igor.stop);
    ...
    const config: BotConfigurations = await this.bot.config;
