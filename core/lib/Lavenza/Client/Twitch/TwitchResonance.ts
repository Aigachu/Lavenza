...
export class TwitchResonance extends Resonance {
  ...
  public async getLocale(): Promise<string> {
    ...
    const i18nUserConfig = await Core.gestalt().get(`/i18n/${this.bot.id}/clients/twitch/users`)
      .catch(Igor.stop);
    ...
    const i18nChannelConfig = await Core.gestalt().get(`/i18n/${this.bot.id}/clients/twitch/channels`)
      .catch(Igor.stop);
