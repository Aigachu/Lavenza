    ...
    static initialize(root = path.dirname(require.main.filename)) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(yield Akechi_1.Akechi.directoryExists(`${Core.paths.bots}/${Core.settings.config.bots.master}`))) {
                yield Morgana_1.Morgana.error(`The configured master bot, "${Core.settings.config.bots.master}", does not exist. Run "lavenza provision" or "lavenza generate bot ${Core.settings.config.bots.master}" to fix this.`);
                yield Morgana_1.Morgana.error("It is highly recommended to run \"lavenza provision\" to make sure that everything is configured correctly!");
                process.exit(1);
            }
