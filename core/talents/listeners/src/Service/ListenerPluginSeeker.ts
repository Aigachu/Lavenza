...
/**
 * Provides a plugin seeker for Listeners.
 */
export class ListenerPluginSeeker extends PluginSeeker<Listener> {
   ...
   */
  protected async plug(plugins: Listener[], entity: Bot | Talent): Promise<void> {
    // Set loaded listeners up in the Listener Catalogue with the sections map.
    await ServiceContainer.get(ListenerCatalogue).storeListenersForEntity(plugins, entity);
  }
